...
colab_instruction = "" if is_colab else """Colab Instruction"""

torch_dtype = torch.float16 if torch.cuda.is_available() else torch.float32
model_id_or_path = "SimianLuo/LCM_Dreamshaper_v7"
device_print = "GPU 🔥" if torch.cuda.is_available() else \
...
if torch.cuda.is_available() or torch.backends.mps.is_available():
    ...
    print(f"Model loaded on {device_print}")
...
def inference(img, source_prompt, target_prompt,
    ...
    if user_instruct != "" and api_key != "":
        source_prompt, target_prompt, local, mutual, replace_steps, num_inference_steps = get_params(api_key,
                                                                                                     user_instruct)
...
def get_params(api_key, user_instruct):
    ...
    return param_dict['source_prompt'], param_dict['target_prompt'], param_dict['target_sub'], param_dict['mutual_sub'], \
    param_dict['attention_control'], param_dict['steps']
...
with gr.Blocks(css=css) as demo:
        ...
        with gr.Column(scale=45):
            with gr.Tab("UAC options"):
                with gr.Group():
                    ...
                    with gr.Row():
                        cross_replace_steps = gr.Slider(label="Cross attn control schedule", value=0.7, minimum=0.0,
                                                        maximum=1, step=0.01)
                        self_replace_steps = gr.Slider(label="Self attn control schedule", value=0.3, minimum=0.0,
                                                       maximum=1, step=0.01)
                    with gr.Row():
            ...
            with gr.Tab("Advanced options"):
                with gr.Group():
                    with gr.Row():
                        num_inference_steps = gr.Slider(label="Inference steps", value=15, minimum=1, maximum=50,
                                                        step=1)
            ...
            with gr.Tab("Instruction following (+GPT4)"):
                ...
                with gr.Group():
                    ...
                    user_instruct = gr.Textbox(label=guide_str,
    ...
    ex = gr.Examples(
        [
            ["images/corgi.jpg", "corgi", "cat", "cat", "", "", "", 1, 2, 15, 512, 512, 0, 1, 0.7, 0.7, 0.6, 0.6,
             False],
            ["images/muffin.png", "muffin", "chihuahua", "chihuahua", "", "", "", 1, 2, 15, 512, 512, 0, 1, 0.65, 0.6,
             0.4, 0.7, False],
            ["images/InfEdit.jpg", "an anime girl holding a pad", "an anime girl holding a book", "book", "girl ", "",
             "", 1, 2, 15, 512, 512, 0, 1, 0.8, 0.8, 0.6, 0.6, False],
            ["images/summer.jpg", "a photo of summer scene", "A photo of winter scene", "", "", "", "", 1, 2, 15, 512,
             512, 0, 1, 1, 1, 0.6, 0.7, False],
            ["images/bear.jpg", "A bear sitting on the ground", "A bear standing on the ground", "bear", "", "", "", 1,
             1.5, 15, 512, 512, 0, 1, 0.3, 0.3, 0.5, 0.7, False],
            ["images/james.jpg", "a man playing basketball", "a man playing soccer", "soccer", "man ", "", "", 1, 2, 15,
             512, 512, 0, 1, 0, 0, 0.5, 0.4, False],
            ["images/osu.jfif", "A football with OSU logo", "A football with Umich logo", "logo", "", "", "", 1, 2, 15,
             512, 512, 0, 1, 0.5, 0, 0.6, 0.7, False],
            ["images/groundhog.png", "A anime groundhog head", "A anime ferret head", "head", "", "", "", 1, 2, 15, 512,
             512, 0, 1, 0.5, 0.5, 0.6, 0.7, False],
            ["images/miku.png", "A anime girl with green hair and green eyes and shirt",
             "A anime girl with red hair and red eyes and shirt", "red hair and red eyes", "shirt", "", "", 1, 2, 15,
             512, 512, 0, 1, 1, 1, 0.2, 0.8, False],
            ["images/droplet.png", "a blue droplet emoji with a smiling face with yellow dot",
             "a red fire emoji with an angry face with yellow dot", "", "yellow dot", "", "", 1, 2, 15, 512, 512, 0, 1,
             0.7, 0.7, 0.6, 0.7, False],
            ["images/moyu.png", "an emoji holding a sign and a fish", "an emoji holding a sign and a shark", "shark",
             "sign", "", "", 1, 2, 15, 512, 512, 0, 1, 0.7, 0.7, 0.5, 0.7, False],
            ["images/214000000000.jpg", "a painting of a waterfall in the mountains",
             "a painting of a waterfall and angels in the mountains", "angels", "", "", "", 1, 2, 15, 512, 512, 0, 1, 0,
             0, 0.5, 0.5, False],
            ["images/311000000002.jpg", "a lion in a suit sitting at a table with a laptop",
             "a lion in a suit sitting at a table with nothing", "nothing", "", "", "", 1, 2, 15, 512, 512, 0, 1, 0, 0,
             0.5, 0.5, False],
            ["images/genshin.png", "anime girl, with blue logo",
             "anime boy with golden hair named Link, from The Legend of Zelda, with legend of zelda logo", "anime boy",
             "", "", "", 1, 2, 50, 512, 512, 0, 1, 0.65, 0.65, 0.5, 0.5, False],
            ["images/angry.jpg", "a man with bounding boxes at the door", "a man with angry birds at the door",
             "angry birds", "a man", "", "", 1, 2, 15, 512, 512, 0, 1, 0.3, 0.1, 0.45, 0.4, False],
            ["images/Doom_Slayer.jpg", "doom slayer from game doom", "master chief from game halo", "", "", "", "", 1,
             2, 15, 512, 512, 0, 1, 0.6, 0.8, 0.7, 0.7, False],
            ["images/Elon_Musk.webp", "Elon Musk in front of a car", "Mark Iv iron man suit in front of a car",
             "Mark Iv iron man suit", "car", "", "", 1, 2, 15, 512, 512, 0, 1, 0.5, 0.3, 0.6, 0.7, False],
            ["images/dragon.jpg", "a mascot dragon", "pixel art, a mascot dragon", "", "", "", "", 1, 2, 25, 512, 512,
             0, 1, 0.7, 0.7, 0.6, 0.6, False],
            ["images/frieren.jpg", "a anime girl with long white hair holding a bottle",
             "a anime girl with long white hair holding a smartphone", "smartphone", "", "", "", 1, 2, 15, 512, 512, 0,
             1, 0.7, 0.7, 0.7, 0.7, False],
            ["images/sam.png", "a man with an openai logo", "a man with a twitter logo", "a twitter logo", "a man", "",
             "", 1, 2, 15, 512, 512, 0, 0.8, 0, 0, 0.3, 0.6, True],
