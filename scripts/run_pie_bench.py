...
def main():
    ...
    for annotation_idx, annotation in annotation_file.items():
        ...
        if annotation["blended_word"] != "":
            ...
            local = ""
        image_out = inference(image_in, source_prompt, target_prompt, "", "", local, "", 1, 2.3, num_inference_steps=12,
                              width=512, height=512, seed=0, strength=1,
                              cross_replace_steps=0.7, self_replace_steps=0.7, eta=1, thresh_e=0.55, thresh_m=0.6,
                              denoise=False)
